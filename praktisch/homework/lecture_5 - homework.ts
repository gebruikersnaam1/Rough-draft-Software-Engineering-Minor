...
type Fun<a,b> = {
  ...
  */
  ...
  // a = option than F<a> = Option<option> which means that map_F cannot that value?, hence F<a> must be just a?
  //            p = F<a>(i.e. Option)
  //            map_F takes A returns F<F<b>> (i.e. Option<option>)
  //            q = so, that means that the first a is a monad (i.e. option) and returns a monand in a monad
  //            so, the correct pseudo code for above is, I think:
  // let bind = <a,b>(p:a, q:Fun<a,F<b>>) : b => map_F<a,F<b>>(q).then(join<b>()).f(p)
  //                    ^remove f, because a and b are monads
  
  let bind_list = function<a, b>(k: Fun<a, List<b>>): Fun<List<a>,List<b>> {
    return Fun<List<a>,List<b>>(x =>  join_list<b>().f(map_list(k,x))) //TODO: add then option to list
  }
  
  type MySet<a> = { var1: a, var2: number }
  let map_Set = <a, b>(f: Fun<a, b>): Fun<MySet<a>, MySet<b>> =>{
    return Fun<MySet<a>,MySet<b>>(x => ({...x, var1: f.f(x.var1)}))  
  }
  
  let unit_Set = <a>(): Fun<a, Set<a>> => null! //zero value is ??? IDK
  
  let join_set = <a>(): Fun<MySet<MySet<a>>, MySet<a>> =>{
    return Fun<MySet<MySet<a>>, MySet<a>>(x => x.var1);
  }
  
  let bind_set = <a, b>(k: Fun<a, MySet<b>>): Fun<MySet<a>,MySet<b>> =>{
    return Fun(x => {
      return join_set<b>().f(map_Set(k).f(x))
    })
  }
  /************************
   * Lesson 5
  ...
  let connect = (ip: string): Option<ServerConnection> => {
    return Math.floor(((Math.random() * ((100 - 0) + 1)) + 0)) >= 16 ? None<ServerConnection>() : Some<ServerConnection>({ip:ip, hello:"h"})
  }
  
  //Some other assignments are yet to be done for lesson 5, but are they worth the trouble? Do I learn something of it
